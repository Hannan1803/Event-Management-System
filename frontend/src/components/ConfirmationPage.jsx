...
const ConfirmationPage = () => {
  ...
  const { event, ticketType, quantity, ticketPrice, totalPrice, ticketDetails } = location.state || {};
  ...
  const handleOkClick = () => {
    Swal.fire({
      ...
      navigate("/my-orders"); // Redirect after alert confirmation
    });
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              <button
                ...
                className="w-full px-4 py-2 bg-black text-white rounded-md hover:bg-white hover:text-black border border-black transition-all"
              >
                {showTicketDetails ? "Hide Ticket Details" : "Show Ticket Details"}
              </button>
            </div>
          </div>

          {showTicketDetails && ticketDetails?.length > 0 && (
            <div className="bg-white p-6 mt-6 rounded-lg shadow-md">
              <h4 className="text-lg font-semibold text-black mb-4 text-center">Ticket Holder Details</h4>
