...
const TicketBooking = () => {
  ...
  earlyBirdDeadline.setDate(eventDate.getDate() - 2);
  earlyBirdDeadline.setHours(0, 0, 0, 0); // Set to midnight

  const today = new Date();
  today.setHours(0, 0, 0, 0); // Set to midnight

  const isEarlyBirdExpired = today > earlyBirdDeadline;

  useEffect(() => {
    if (isEarlyBirdExpired && formData.ticketType === "earlybird") {
      setFormData((prev) => ({ ...prev, ticketType: "general" }));
      setTicketPrice(ticketTypes.general);
    }
  }, [isEarlyBirdExpired, formData.ticketType]); // ✅ Added dependency
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (ticketDetails.length < formData.quantity) {
      alert("Please add details for all tickets before proceeding.");
      return;
    }
        ...
        <div className="flex gap-8">
          {Object.entries(ticketTypes).map(([key, price]) => (
            <label key={key} className={`border-2 p-4 w-1/3 flex justify-center items-center rounded-lg cursor-pointer ${formData.ticketType === key ? 'bg-black text-white' : ''}`}>
              <input type="radio" name="ticketType" value={key} checked={formData.ticketType === key} onChange={handleChange} className="hidden" disabled={key === "earlybird" && isEarlyBirdExpired} />
              <div className={`text-xl font-bold ${key === "earlybird" && isEarlyBirdExpired ? 'text-red-500' : ''}`}>
                <div className="text-xl font-bold">{capitalizeFirstChar(key)}</div>
                <div className={`text-lg ${key === "earlybird" && isEarlyBirdExpired ? 'text-red-500 font-semibold' : ''}`}>
                  {key === "earlybird" && isEarlyBirdExpired ? 'Expired' : `$${price}`}
                </div>
              </div>
            </label>
          ))}
